...
public class Asts {
	...
	public static AssignStmt init(String scope) {
		return new AssignStmt(new NameExpr(new Name(scope + "_counter")),
				integer(0));
	}
	...
	public static AssignStmt increment(String scope) {
		return new AssignStmt(new NameExpr(new Name(scope + "_counter")),
				new PlusExpr(new NameExpr(new Name(scope + "_counter")),
						integer(1)));
	}
	...
	private static Expr integer(int i) {
		return new IntLiteralExpr(new DecIntNumericLiteralValue(
				String.valueOf(i)));
	}
	...
	// Note: an alternative would be to parse the little snippets you need.
	// Maybe useful
	// exercise: write a method ASTNode<?> parse(String code) that would let you
	// write the
